...
export async function POST(req: NextRequest) {
  try {
    ...
    // ----------------------------------------------------
    // 2.2b Command /pwa or text "pwa", "установить"
    // ----------------------------------------------------
    if (text === '/pwa' || text === 'pwa' || text === 'установить' || text === '/install') {
      const cleanName = fromUser.first_name || 'Пользователь';
      const pwaDirectUrl = `${APP_URL}/?auth_id=${fromUser.id}&couple=${userCoupleCode || ''}&name=${encodeURIComponent(cleanName)}`;

      const pwaText = `✨ <b>Как установить «Мы Вместе» как приложение на телефон (PWA)?</b>\n\n` +
        `💡 <b>Что такое PWA?</b>\n` +
        `Это полноценное приложение, которое работает прямо с экрана вашего смартфона как отдельная программа (как из App Store / Google Play), но без необходимости скачивания и без постоянного открытия Telegram!\n\n` +
        `🛒 <b>Пример из жизни:</b>\n` +
        ...
        `1️⃣ <b>Нажмите кнопку «Установить на телефон»</b> внизу этого сообщения — откроется браузер с вашим профилем.\n\n` +
        ...
        `3️⃣ <b>Готово! 🎉</b>\n` +
        `На экране телефона появится иконка «Мы Вместе». Ваш профиль <b>${cleanName}</b> и пара подключатся автоматически!`;

      await sendTelegramMessage(chatId, pwaText, {
        inline_keyboard: [
          [
            {
